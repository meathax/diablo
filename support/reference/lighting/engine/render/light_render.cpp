...
// The tile-light array is small (~12 KiB). Compare actual contents so every
// producer of lighting changes participates without a new invalidation API.
struct LightmapCache {
	bool valid = false;
	Point tilePosition {};
	Point targetPosition {};
	int width = 0, height = 0, rows = 0, columns = 0;
	uint_fast8_t microTileLen = 0;
	uint8_t tileLights[MAXDUNX][MAXDUNY] {};
	int uniformLevel = -1;
#ifdef DIABLO_LIGHTING_TESTING
	uint64_t builds = 0;
#endif
};
LightmapCache CachedLightmap;
...
{
	if (perPixelLighting) {
		auto &cache = CachedLightmap;
		const bool unchanged = cache.valid
		    && cache.tilePosition == tilePosition && cache.targetPosition == targetBufferPosition
		    && cache.width == viewportWidth && cache.height == viewportHeight
		    && cache.rows == rows && cache.columns == columns && cache.microTileLen == microTileLen
		    && std::memcmp(cache.tileLights, tileLights, sizeof(cache.tileLights)) == 0;
		if (!unchanged) {
			BuildLightmap(tilePosition, targetBufferPosition, viewportWidth, viewportHeight, rows, columns, tileLights, microTileLen);
			cache.tilePosition = tilePosition;
			cache.targetPosition = targetBufferPosition;
			cache.width = viewportWidth;
			cache.height = viewportHeight;
			cache.rows = rows;
			cache.columns = columns;
			cache.microTileLen = microTileLen;
			std::memcpy(cache.tileLights, tileLights, sizeof(cache.tileLights));
			cache.uniformLevel = !LightmapBuffer.empty()
			        && std::adjacent_find(LightmapBuffer.begin(), LightmapBuffer.end(), std::not_equal_to<uint8_t> {}) == LightmapBuffer.end()
			    ? LightmapBuffer.front() : -1;
			cache.valid = true;
#ifdef DIABLO_LIGHTING_TESTING
			++cache.builds;
#endif
	...
	Lightmap result(outBuffer, outPitch, LightmapBuffer, viewportWidth, lightTables, fullyLitLightTable, fullyDarkLightTable);
	if (perPixelLighting) {
		result.uniformLightLevel_ = CachedLightmap.uniformLevel;
		if (const auto *table = result.uniformLightTable(); table != nullptr) {
			result.uniformLightIdentity_ = true;
			for (size_t i = 0; i < LightTableSize; ++i) {
				if (table[i] != i) {
					result.uniformLightIdentity_ = false;
					break;
				}
			}
		}
...
Lightmap Lightmap::bleedUp(bool perPixelLighting, const Lightmap &source, Point targetBufferPosition, std::span<uint8_t> lightmapBuffer)
{
	...
	if (!perPixelLighting || source.uniformLightTable() != nullptr) return source;
...
#ifdef DIABLO_LIGHTING_TESTING
uint64_t Lightmap::buildCountForTesting() { return CachedLightmap.builds; }
#endif
