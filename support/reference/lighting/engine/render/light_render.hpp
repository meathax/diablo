...
public:
	...
	const uint8_t *getLightingAt(const uint8_t *outLoc) const
	{
		...
		if (outDist >= 0 && outPitch == lightmapPitch)
			return lightmapBuffer.data() + outDist;
	...
	// Uniformity describes the generated pixels, including clipping and borders.
	[[nodiscard]] const uint8_t *uniformLightTable() const
	{
		return uniformLightLevel_ < 0 ? nullptr : lightTables[uniformLightLevel_].data();
	}
	[[nodiscard]] bool uniformLightIsIdentity() const { return uniformLightIdentity_; }
#ifdef DIABLO_LIGHTING_TESTING
	static uint64_t buildCountForTesting();
#endif
...
private:
	...
	int uniformLightLevel_ = -1;
	bool uniformLightIdentity_ = false;
};
