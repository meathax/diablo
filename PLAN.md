...
> **Authority and migration, 8 September 2026:** this document is the root `PLAN.md` authority. The complete C01–C34 and R01–R13 acceptance procedures remain intact. `PROGRESS.md` carries only the concise current status and next actions; scheduling/history narrative is retained in `reports/audit-2026-09-07/archive/PLAN_HISTORY.md`, and the former historical guide is retained under the same archive.

## Controller mapping contract — custom Xbox preset

The user-specified custom preset below is the controller-only behavior contract for Diablo 1 on MiSTer. Every letter names the physical Xbox button. It builds on the engine's separate attack, interaction, casting, potion and stand-ground actions and is accepted only when the gameplay and panel contexts below are exercised on the same candidate.

### Gameplay bindings

| Xbox control | Binding |
| --- | --- |
| Left stick | Move character. |
| Right stick | Precision mouse cursor for manual targeting and detailed inventory interaction. |
| A | Attack / talk; hold to continue attacking; confirm when a menu is open. |
| X | Cast the currently selected spell; hold for normal repeated casting where applicable. |
| Y | Pick up loot / open doors / open chests / interact with objects. |
| B | Open quick spell selection during gameplay; cancel / back / close when a panel is open. |
| LB | Use a healing item from the belt, one activation per press. |
| RB | Use a mana item from the belt, one activation per press. |
| LT (hold) | Stand ground: attack or cast without walking out of position; release to move normally. |
| RT (hold) | Quick-spell modifier: RT+A/B/X/Y casts the assigned spell; RT alone does nothing to movement. |
| D-pad up / right / down / left | Spellbook / inventory / quest log / character sheet and allocate attributes. |
| View (overlapping squares) | Toggle automap. |
| Menu (three lines) | Open game menu: save, load, options, quit. |
| L3 | Toggle ground-item labels; one press toggles, no hold required. |
| R3 | Left mouse click at the precision cursor. |
| RT+R3 | Right mouse click at the precision cursor. |
| Xbox / Guide | Reserved for the MiSTer OSD where input integration exposes it; no Diablo action. |

### RT quick-spell layer

X without RT remains the selected spell. The four assignable native quick-spell slots are:

| Combination | Suggested role | Native slot |
| --- | --- | --- |
| RT+X | Alternative offensive spell | `QuickSpell1` |
| RT+Y | Crowd-control or defensive spell | `QuickSpell2` |
| RT+A | Healing spell | `QuickSpell3` |
| RT+B | Utility spell | `QuickSpell4` |

The roles are suggestions, not hard-coded spell choices; all four slots remain assignable per character. `Quick Cast` is on by default in this preset so the combinations cast immediately. B opens quick spell selection; while that screen is open, highlight a spell and press its RT+face-button combination to assign it. RT's standalone action remains unbound. Bind these as direct `QuickSpell1`–`QuickSpell4` combinations rather than using the existing `PadHotspellMenu` action: RT alone must not block movement. RT+D-pad pans the visible automap, overriding the D-pad panel shortcuts only for that combination. `Auto Refill Belt` and `Auto Gold Pickup` are on by default in this preset. A small four-icon RT overlay is a useful port-side addition and remains implementation scope, not a native-remapping assumption.

### In-game MiSTer OSD reference

The core OSD must expose a clearly named **Show controls** entry. Selecting it opens one native MiSTer-style subpage with the complete physical-gamepad reference above: face buttons, shoulders/triggers, RT quick-spell layer, sticks/clicks, D-pad/pan, View, Menu and Guide. It must contain no keyboard, mouse or implementation bindings; keep the page to one unscrolled screen at the standard OSD scale. The mapping stays descriptive only: it never changes controller state or assigns another action to RT alone.

### Context, modifier and safety behavior

| Context | Required behavior |
| --- | --- |
| Inventory or menu navigation | D-pad and left stick navigate entries or inventory cells; menus suppress character movement and panel hotkeys. A confirms and picks up/places; B cancels, goes back or closes, and while carrying an item restores the move safely without dropping it; Y uses/equips or quick-transfers inventory↔stash; X quick-moves to equipment/belt and, while carrying an item, requires a deliberate hold to drop it. |
| Stash | LB/RB select the previous/next stash page and suppress potion use. |
| Modifier and release order | Remove conflicting defaults, including D-pad movement, LT/RT panel shortcuts and old Start/Select modifiers; menus have priority; modified presses are consumed; releasing an RT combination must not also invoke its ordinary face-button action, whether RT or the face button is released first. |
| Precision and usability | Keep native controller targeting for ordinary combat; retain the right-stick cursor as the precision fallback for targeting and detailed inventory work; use the ground-label toggle, Quick Cast, Auto Refill Belt and Auto Gold Pickup to reduce routine work. Safe cancellation of a carried item must never drop it accidentally. |

### Implementation and acceptance milestones (C10–C12/C24)

1. **C10/C11 — focus and modifiers:** encode the exact gameplay/panel map, remove the listed conflicting defaults, give menus priority, suppress movement and panel hotkeys in menus, reconcile held controls across panel transitions, consume RT-modified presses through both release orders, and test every face-button/RT press and release order. Verify direct quick-spell bindings rather than `PadHotspellMenu`, RT alone does not move or block the character, and Guide remains available for the MiSTer OSD.
2. **C12 — cursor and mouse masks:** verify right-stick precision cursor movement, R3 left click, RT+R3 right click, RT+D-pad automap pan and panel-specific suppression; exercise targeting and inventory interaction without leaking clicks or movement into the active panel.
3. **C24 — controller-only workflows:** on the candidate, complete combat, talk, loot, doors/chests, selected and four quick spells, potions, stand-ground, spellbook, inventory, quest log, character allocation, automap, save/load/options/quit, stash paging/transfer and safe carried-item cancellation/drop with no keyboard dependency. Repeat the relevant panel and modifier cases in the controller-only multiplayer workflow.
...
5. **Acceptance evidence:** retain the exact preset/configuration, a context matrix covering gameplay, menus, inventory, stash, automap, quick-spell assignment and Guide/OSD, release-order results, and implementation/validation evidence for the four-icon RT overlay and one-page **Show controls** OSD reference, together with candidate-bound controller-only and multiplayer receipts. A mapping table alone does not close C10–C12 or C24.
...
[current audit refresh](reports/audit-2026-09-07/REFRESH_AUDIT.md) adds R01–R13 with exact evidence,
...
requirements are in `reports/audit-2026-09-07/REFRESH_AUDIT.md`.
...
**Accepted diagnostic implementation (2026-09-08, commit `9bf880a`):** PCM RTL writes a 48-byte snapshot at ABI offset 424 only on the first active starvation edge, clears its commit word on reset/reattach, and publishes the payload before the commit marker. ARM reads it with commit/epoch coherence checks and reports unavailable for an older FPGA image. Independent player/long/arbiter/integrated RTL, host ABI and WSL state-dump tests pass. This changes measurement only; no playback tuning, hardware audio or release acceptance is claimed. Reproducible FPGA compile/timing/compress is delegated in `.work/goal-pcm-fpga-9bf880a`; C15 remains open for candidate-bound build, 30-minute active Diablo/Hellfire traces, physical stereo inspection and zero steady underrun/resync/drop deltas.
...
**Input-lag reduction worklist (user request, 8 September 2026)**

These are implementation/measurement tasks, not claims of achieved latency reduction. Preserve the existing physical targets of p95 input-to-visible at most **50 ms** and p99 at most **100 ms**, along with the 60 FPS/cadence and correctness gates.

| Priority | Concrete opportunity and current evidence | Required implementation and validation |
| --- | --- | --- |
| 1 | Input service is coupled to rendering: `support/reference/mister_transport_sdl.hpp` calls `PumpInput()` from `Present()`, with at most 64 events per call. | Add bounded input service before the engine drains SDL events, independent of publication. Measure capture-to-drain and drain-to-action ages before/after; preserve all button edges, modifier release orders, mouse deltas, focus loss and overflow recovery. Avoid blocking the audio callback or busy-spinning. |
| 2 | Frame pacing runs after presentation; forced timedemo pacing can sleep until a 60 Hz deadline. | Measure event arrival during limiter sleeps and service input around deadlines where useful. Keep one pacing authority, avoid a second frame of waiting, and preserve native simulation timing, replay determinism, audio service and 60 Hz presentation. The engine's default 50 ms simulation tick is not permission to speed up gameplay. |
| 3 | Three frame slots feed oldest-ready/FIFO selection at vblank (`rtl/diablo_framebuffer_scanout.sv`). | Record age of the input associated with each displayed frame and queue depth. Compare bounded producer queue depth against safe latest-ready selection. Latest-ready requires explicit retirement of every superseded slot/fence and palette pairing; never overwrite a displayed/in-flight frame or merely discard completion accounting. Retain C03/C04 ownership and cadence tests. |
| 4 | Full-frame copies move 640×480 indexed pixels; dirty copying and a copy benchmark already exist. | Run paired scene-specific full/dirty-copy and present-stage measurements, then reduce the measured copy/publication bottleneck. Verify exact pixels/palette and p99 timings under gameplay/audio/input contention. Historical dummy-SDL copy/FPS figures cannot establish current physical latency or justify enabling the optional command path's 25 ms fence. |
| 5 | The FPGA captures ordinary input edges immediately. Its 50,400-cycle/1 ms timer refreshes the consumer cursor under pressure; ARM batches can leave a backlog. | Stress bursts, mouse motion and controller chords. Tune batch size, cursor refresh or DDR arbitration only when traces show backlog/service delay, while retaining bounded work, audio priority, no missing releases and no input overflow. Do not advertise a normal-input improvement from shortening a timer that normal edges bypass. |
| 6 | HDMI scanout/presentation and the external display add latency beyond ARM transport timing. | Measure button-to-visible response on the supported HDMI setup, record Main/kernel, display mode and any scaler/display latency settings, and compare only supported configurations with identical gameplay. Retain vblank synchronization and tear-free output; do not claim CRT/direct-video measurements under the HDMI-only test scope. |

First add an input-to-frame trace joining FPGA event sequence/timestamp, ARM drain, SDL consumption, action tick, frame publication and actual displayed frame/vblank. Calibrate clock domains and account for dropped/overwritten telemetry. Use a repeatable physical trigger/capture for end-to-end results; software timings remain explicitly transport-only. Rank optimizations by measured contribution, use at least three paired runs per affected scene/device, and report p50/p95/p99/p99.9/max, queue age, deadline misses, drops and CPU/audio effects. A faster average with worse tails or lost input does not close this worklist. Retain the original C21 thresholds and all C10–C12 controller safeguards, including deliberate hold-to-drop behavior.

**Build-throughput policy (2026-09-08):** Host-only controller regression uses `DISABLE_LTO=ON` and `DISABLE_ZERO_TIER=ON` in `support/host-reference.json`, avoiding Windows link-time-optimization delays and an irrelevant multiplayer-library link path without changing an ARM or FPGA candidate. The release/target builds retain their network configuration. Batch compatible controller and OSD source edits, run their focused static/unit checks before creating one immutable FPGA snapshot, then perform one compile, timing, compression and deployment pass. This shortens iteration only; no candidate-bound or physical acceptance check is skipped.
...
**Recorded implementation (2026-09-08, committed source packets):** P01-P06 are committed through `a65a8ec0708fcbc0d4c3e390fc2c0056d935c437`. The durable packet records are [the source-packet classification](reports/audit-2026-09-07/refresh-evidence/c26-source-packet-classification.json) (SHA-256 `ef21ea107eea6f3f3c6a381e6024e54ff97220c1bf35b2ad39f01b803c1242e2`) and [the commit ledger](reports/audit-2026-09-07/refresh-evidence/c26-commit-ledger.json) (SHA-256 `07fb9d6de77f77281544e09bf7f846ab18738a2fb372875f6fc729053bd30e28`), copied byte-identically from the private work ledger. C26 remains VERIFYING until the current docs/evidence packet and clean-checkout qualification are accepted.
