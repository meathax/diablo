...
- **In progress:** clean-checkout verification repair and endpoint-aligned ARM replay validation. Scope, diagnostic, provenance and replay-preflight packets are committed and independently reviewed through `3c7888a`. Development candidate `b537b12c1dc8afbd36cb2c5e82d0a58cff4be2f50ca60d0cf38badb540b49c7b` has user-confirmed gameplay video and audio; no accepted release exists.
...
- **Open:** PCM underflow telemetry attribution and endurance qualification; remaining C01–C34/R01–R13 integrated/release evidence, including C25 external board/timing contracts. The user reports good audio; counters alone do not establish an audible defect. Missing Git remote blocks the historical push only.
...
- **Provenance/preflight accepted:** commit `82ceac1` validates the retained ARM-reference build through typed, re-derived recipe/source/build-log/artifact evidence (16 focused tests and independent review pass); it proves historical local provenance only, not a new clean build. Commit `3c7888a` admits replay assets before QEMU, verifies optional manifest paths/hashes, rejects traversal/symlink escapes, and passes five WSL tests plus independent review. The third exact repaired-binary run validated 184 assets and reached comparison, then reported differing saved state and timed out; retain `.work/goal-replay-terra/replay-evidence.json` and all three failed runs. The donor and fixture revision both match `0ff318…`; `d2ccbc3` is the separate integration commit. Headless execution, configuration and data/oracle compatibility remain unproven, so no engine regression is established. Next compare an identity- and mode-aligned reference/transport pair; do not replace expected output or repeat the same run blindly.
...
- **Replay A/B review:** `.work/goal-replay-ab/replay-ab-evidence.json` preserves the passing native headless oracle and two incomplete CLI runs. Independent source/fixture review establishes 4,854 fixture GameTicks; reference stopped at 2,309 logical ticks and transport at 1,500, both timeout 124. Shutdown comparison writes stable saves at those different checkpoints. Archive differences do not establish an engine regression. Next prove full fixture consumption and the same explicit logical endpoint before comparison; preserve expected output.
- **PCM trace review:** `.work/goal-pcm-analysis/report-f0a2087f.md` and `evidence-f0a2087f.json` explain four FIFO-empty counter jumps in 120 samples, with advancing producer/callback/display, zero callback drops and zero resyncs. Positive sampled occupancy cannot resolve between-sample empty events. The user hears good audio; C15 remains open. Smallest causal measurement is underflow-time queue/fetch/producer/player/DDR-arbiter state capture, before any tuning.
- **Clean-checkout verification:** isolated `3c7888a` foundation/local runs fail preflight because ignored generated `build_id.v` is missing. Focused tests also reveal historical receipt prerequisites and a stale/line-ending-sensitive plan digest. Repair source-only generation with explicit receipt provenance while retaining strict candidate validation; make synthetic tests hermetic and pin plan LF. No integrated pass is claimed yet.
