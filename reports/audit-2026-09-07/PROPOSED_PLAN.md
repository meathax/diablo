...
This update supersedes the earlier window/not-activated status below. The user authorized MiSTer testing; C26 is activated as a development-only, pre-fix baseline. The user confirms animated title and good audio, followed after approximately 30 seconds by a frozen title while cinematic audio plays. Source tracing proves the generic ARM presentation hook republishes `PalSurface` while movies update a separate indexed `SVidSurface`. The persistent engine and FPGA acknowledgements continue: this is not an ended test or proof of queue deadlock.

- Next: finish the incremental ARM build and package verification, then bind a new immutable development candidate using the qualified FPGA and unchanged assets/ABI and run the full candidate-bound title → cinematic → title cycle. The generated movie hook and conversion helper are implemented; the focused source/test packet is committed in d2ccbc3, and the executable helper/adapter regression plus static generated-hook review pass. Full generated ARM compilation and hardware verification remain open. No Quartus run is justified by this ARM-only defect.
...
- Acceptance: changing indexed movie pixels and palettes against a static title buffer, forced backpressure, disabled transport, and return to title through actual integration; then observe a full title → cinematic → title cycle on the new candidate. A short startup smoke cannot close this blocker. Equal sampled descriptor CRCs alone do not prove final output is frozen.
...
- Evidence: `.work/c26/frame-backpressure-diagnosis.md`; current candidate `8530508c0923c8d14766f584019bb4bd5b78ed614c95facb474d5b6788ed1376` remains the pre-fix development baseline.
