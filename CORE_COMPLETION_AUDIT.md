...
- **Current development candidate:** `b537b12c1dc8afbd36cb2c5e82d0a58cff4be2f50ca60d0cf38badb540b49c7b`, package `.work/package-board-c26-cinematic-d2ccbc3-assets`, target `/media/fat/_CodexDiabloCinematic_d2ccbc3`. The repaired runtime was left running; last verified launcher/engine PIDs were 2515/2524, boot `0cd4f2ef-9ab9-4653-bc72-cba05f3490e2`. Revalidate live ownership before interacting; do not infer that historical PIDs remain live. Earlier boot text with `cbaa05` was a transcription error. Private game data is verified at `/media/fat/_CodexDiabloGameData`; preserve user data and existing test artifacts.
- **Visual blocker:** user confirmation of the repaired title → cinematic → title cycle is still pending. Changing full-frame CRCs, advancing frames and zero conversion diagnostics are useful telemetry, not physical visual acceptance. Do not disable cinematics or promote the candidate on title-only evidence.
- **Audio investigation:** the final passive trace had 90 samples, ready states and fault 0. Producer/consumer each advanced 3,802,872; display frames advanced 1,307. Thirteen intervals had no PCM/display advance. Six intervals had differing producer/consumer deltas; three samples had absolute mismatches (maximum 2,229). Underruns increased in eight intervals and then reset from 121,171 to 0, so a cumulative delta across that reset is invalid. The installed dump omits `queued_frames` and `local_queue_frames`; starvation versus intentional/producer pauses remains unresolved. No audible fault was confirmed. Evidence: `.work/c26/cinematic-pcm-passive-trace-b537b12c.summary.json`, SHA-256 `26961f57e588067e0ecd600389eabacdc202ce0bf3a818587de15964ac5a37f3`, with sibling raw trace. Next diagnose queue levels and reset/audio-production boundaries before changing PCM code. Consider a controlled diagnostics-on/off comparison only after preserving the user's visual observation.
- **Next independent checks, not run:** candidate-bound deployment lifecycle; repaired ARM replay (WSL QEMU 10.2.1 is now available, but this binary's replay remains unrun); then candidate-bound campaign/save/input checks. `scenario_arm.py` currently hardcodes an older build and needs a scoped build-directory parameter before it can prove this candidate. Host-reference tests alone do not close ARM/physical gates.
